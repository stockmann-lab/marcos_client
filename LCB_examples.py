...
if __name__ == "__main__":
    ...
    rxds = []
    for k in range(10):
        rxd = turbo_spin_echo(lo_freq=15.46,
            trs=1, echos_per_tr=32,
            rf_amp=0.35,
            rx_period=25/3,
            echo_duration=5000,
    ...
    tr_mat = np.reshape(rxd_av, (-1, rxd_av.size//32) )
    trfft = np.fft.fftshift(np.fft.fft2(tr_mat))
    #import pdb;pdb.set_trace()

    plt.subplot(3,1,1)
    plt.plot(np.abs(rxd))
    plt.subplot(3,1,2)
    plt.plot(np.abs(tr_mat))
    #plt.subplot(3,1,3)
    plt.figure()
    plt.imshow(np.abs(trfft))
    plt.show()
