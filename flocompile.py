...
def csv2bin(path, quick_start=False, initial_bufs=np.zeros(FLOCRA_BUFS, dtype=np.uint16), latencies = np.zeros(FLOCRA_BUFS, dtype=np.int32)):
...
def dict2bin(sd, initial_bufs=np.zeros(FLOCRA_BUFS, dtype=np.uint16), latencies = np.zeros(FLOCRA_BUFS, dtype=np.int32)):
...
def cl2bin(changelist, changelist_grad,
           initial_bufs=np.zeros(FLOCRA_BUFS, dtype=np.uint16)):
    ...
    def cl2ol(changelist):
        ...
        def close_timestep(time):
            ...
            # buf_time_offsets = np.zeros(FLOCRA_BUFS, dtype=int32)
            buf_time_offsets = 0
    ...
    # buf_empty_time = np.zeros(FLOCRA_BUFS, dtype=np.int32)
    debug_print("changes:")
    for k in changes:
        debug_print(k)
